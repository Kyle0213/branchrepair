...
def _sample_cases(cases: list[dict], sample_size: int, sample_seed: int) -> list[dict]:
    if sample_size <= 0 or sample_size >= len(cases):
        return list(cases)
    rng = random.Random(sample_seed)
    selected = rng.sample(list(enumerate(cases)), sample_size)
...
def main() -> None:
    ...
    parser.add_argument("--sample_size", type=int, default=0, help="Randomly sample this many Bamboogle cases before generating fresh round0. Use 0 for full dataset.")
    ...
    all_cases = _load_json(args.cases_path)
    cases = _sample_cases(all_cases, args.sample_size, args.sample_seed)
    ...
    summary = {
        "dataset": "bamboogle",
        "num_cases": len(cases),
        "source_num_cases": len(all_cases),
